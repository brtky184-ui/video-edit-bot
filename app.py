...
async def start(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    await u.message.reply_text("🚀 Stil güncellendi! Videoyu, SESİ ve Altyazıyı gönder, gerisini bana bırak.")
...
async def catch(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    await u.message.reply_text(f"✅ Alındı: {path.split('_')[-1]}")
...
async def merge(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    if not all([d.get('v'), d.get('a'), d.get('s')]):
        await u.message.reply_text("❌ Dosyalar hala eksik!")
        return

    m = await u.message.reply_text("🎬 Profesyonel render başladı, bilgisayarın kapalı olsa da ben buradayım...")
    out = f"final_{uid}_{int(time.time())}.mp4"
    sub_path = os.path.abspath(d['s'])
    
    # EKRANA KİTLEYEN STİL: Sarı renk, Kalın font, Siyah kenarlık
    style = "FontSize=24,PrimaryColour=&H00FFFF&,OutlineColour=&H000000&,BorderStyle=1,Outline=2,Bold=1"
    ...
    try:
        ...
        await u.message.reply_document(document=open(out, 'rb'), caption="İşte o video! 🔥")
    except Exception as e:
        await u.message.reply_text(f"Hata: Formatı kontrol et!")
